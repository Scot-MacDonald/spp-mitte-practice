...
const getMediaUrl = (metaImage: any) => {
  ...
  // Extract a stable version key if available (Payload stores _updatedAt or updatedAt)
  const version =
  ...
  // Case: Simple filename from Payload
  if (typeof metaImage === "string") {
    return version
      ? `/api/media/file/${metaImage}?v=${version}`
      : `/api/media/file/${metaImage}`;
  }

  // Case: Payload Media object with its own url
    ...
    <Link href={href} className={cn("block", className)}>
      <article className="border border-border rounded-lg p-4 transition-colors hover:bg-[rgba(126,179,106,0.1);]">
        ...
        <div className="py-4">
          ...
          {titleToUse && (
            <h3 className="text-base font-bold text-gray-700">{titleToUse}</h3>
          )}
